...
export const emails: AreaDict = {
  en: {
    ...
    // The end-of-week scoreboard. Ten rotating headlines, picked at random when the recap row
    // is created and then frozen, so a retry never rewords an email that may already be out.
    'emails.weekRecap.line.0': '{name}, your average is now {average}%',
    'emails.weekRecap.line.1': 'Week closed — {name} is averaging {average}%',
    'emails.weekRecap.line.2': 'That is a wrap, {name}. Running average: {average}%',
    'emails.weekRecap.line.3': '{name}, the scoreboard says {average}%',
    'emails.weekRecap.line.4': 'Another week down — {name} sits at {average}%',
    'emails.weekRecap.line.5': '{name}, you are averaging {average}% across the cycle so far',
    'emails.weekRecap.line.6': 'Week in the books: {name} is at {average}%',
    'emails.weekRecap.line.7': 'Here is where you stand, {name}: {average}%',
    'emails.weekRecap.line.8': '{name}, your cycle average just landed at {average}%',
    'emails.weekRecap.line.9': 'The week is done — {name} is holding {average}%',
    'emails.weekRecap.eyebrow': 'Weekly scoreboard',
    'emails.weekRecap.preheader': 'Your average across the cycle so far is {average}%.',
    'emails.weekRecap.intro': 'Week {week} is over and finished at {score}%. Here is every completed week so far.',
    'emails.weekRecap.tableCaption': 'Your cycle so far',
    'emails.weekRecap.weekLabel': 'Week {week}',
    'emails.weekRecap.averageLabel': 'Average',
    'emails.weekRecap.onTrackTitle': 'You are on track',
    'emails.weekRecap.onTrackText': 'Your average is at or above the {target}% target. Keep the same pace next week.',
    'emails.weekRecap.offTrackTitle': 'Room to climb',
    'emails.weekRecap.offTrackText': 'The target is {target}%. One extra completed tactic a day is usually all it takes to close that gap.',
    'emails.weekRecap.footer': 'Sent once a week, after the week ends.',
  ...
  he: {
    ...
    // סיכום סוף השבוע. עשרה ניסוחים מתחלפים, נבחרים באקראי בעת יצירת השורה ואז מוקפאים.
    'emails.weekRecap.line.0': '{name}, הממוצע שלך עכשיו {average}%',
    'emails.weekRecap.line.1': 'השבוע נסגר — הממוצע של {name} הוא {average}%',
    'emails.weekRecap.line.2': 'זהו, {name}. הממוצע המצטבר: {average}%',
    'emails.weekRecap.line.3': '{name}, לוח התוצאות אומר {average}%',
    'emails.weekRecap.line.4': 'עוד שבוע מאחורינו — {name} עומד על {average}%',
    'emails.weekRecap.line.5': '{name}, הממוצע שלך במחזור עד כה הוא {average}%',
    'emails.weekRecap.line.6': 'השבוע נסגר: {name} על {average}%',
    'emails.weekRecap.line.7': 'הנה איפה אתה עומד, {name}: {average}%',
    'emails.weekRecap.line.8': '{name}, ממוצע המחזור שלך נחת על {average}%',
    'emails.weekRecap.line.9': 'השבוע הסתיים — {name} מחזיק {average}%',
    'emails.weekRecap.eyebrow': 'לוח התוצאות השבועי',
    'emails.weekRecap.preheader': 'הממוצע שלך במחזור עד כה הוא {average}%.',
    'emails.weekRecap.intro': 'שבוע {week} הסתיים ונסגר על {score}%. הנה כל השבועות שהושלמו עד כה.',
    'emails.weekRecap.tableCaption': 'המחזור שלך עד כה',
    'emails.weekRecap.weekLabel': 'שבוע {week}',
    'emails.weekRecap.averageLabel': 'ממוצע',
    'emails.weekRecap.onTrackTitle': 'אתה בקצב',
    'emails.weekRecap.onTrackText': 'הממוצע שלך עומד ביעד {target}% או מעליו. שמור על אותו קצב גם בשבוע הבא.',
    'emails.weekRecap.offTrackTitle': 'יש לאן לטפס',
    'emails.weekRecap.offTrackText': 'היעד הוא {target}%. בדרך כלל מספיקה טקטיקה אחת נוספת ביום כדי לסגור את הפער.',
    'emails.weekRecap.footer': 'נשלח פעם בשבוע, אחרי שהשבוע מסתיים.',
